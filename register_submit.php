...
try {
  ...
  if(is_null($_POST['TeamEmail']) || $_POST['TeamEmail'] == '') {
    $db_error_msg = 'An email address is required.';
    throw new Exception('');
  }
  ...
  $pdo->prepare($sql)->execute([$_POST['TeamName'], $_POST['TeamAbbr'], $_POST['TeamSchool'], $_POST['TeamEmail'], $agreed]);
  ...
  foreach(range(1, 6) as $i) {
    if(isset($_POST["Member${i}Name"]) && $_POST["Member${i}Name"] != '' ) {
      ...
      $statement->execute([$_POST["Member${i}Name"], $i == 1 ? 'Captain' : 'Member', $_POST["Member${i}Size"]]);
    }
    else {
      if($i == 1) {
